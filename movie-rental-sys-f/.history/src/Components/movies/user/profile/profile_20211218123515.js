...
export default function UserProfile(){
    ...
    useEffect(()=>{
        ...
        .then((response) => {
            console.log("hiii",response.data.userInfo);
            ...
            <div className="container">
                ...
                {userDetails.map((data) =>{
                    if(data === "no_user"){
                        ...
                        return(
                            <div className="col-md-3" key={userDetails[0]._id}>
                                <div className ="card">
                                    <div className="card-header text-center card-header-color" >
                                        <h5 className="data-title" >{userDetails[0].name}</h5>
                                    </div>
                                    <div className="card-body">
                                        <p className="email">Email: {userDetails[0].email}</p>
                                        <p className ="role">Role: {userDetails[0].role}</p>
                                        <p className ="rents">Total rents: {userDetails[0].rent}</p>
                                    </div>
                                </div>
                            </div>
                        )
