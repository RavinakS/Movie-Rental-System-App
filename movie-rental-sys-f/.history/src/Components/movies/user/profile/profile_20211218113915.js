...
export default function UserProfile(){
    ...
    useEffect(()=>{
        ...
        .then((response) => {
            console.log("hiii",response.data.userRents);
            setUserDetails(response.data.userRents);
        })
        .catch((err) =>{
            try{
                ...
                {userDetails.map((data) =>{
                    ...
                    if(data === "no_user"){
                        ...
                        if(!check){
                            ...
                            check = true
                            return  (
                                <div className="col-md-3" key={userDetails[1][0].movie_id._id}>
                                    <div className ="card">
                                        <div className="card-header text-center card-header-color" >
                                            <h5 className="data-title" >{userDetails[1][0].movie_id.name}</h5>
                                        </div>
                                        <div className="card-body">
                                            <p className="genre">genre: {userDetails[1][0].movie_id.genre}</p>
                                            <p className ="releasDate">releasDate: {userDetails[1][0].movie_id.releasDate}</p>
